...
import { assertWorkspaceMember, updateCard, getBoard } from '../../../../../lib/features/board/dynamodb'
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { workspaceId, channelId, cardId, updates } = body ?? {}

    if (!workspaceId || !channelId || !cardId) {
      return NextResponse.json(
        { error: 'workspaceId, channelId and cardId are required' },
        { status: 400 },
      )
    }
    ...
    const card = await updateCard({ workspaceId, channelId, cardId, updates: updates ?? {} })
