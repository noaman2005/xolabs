...
import { assertWorkspaceMember, createCard, getBoard } from '../../../../../lib/features/board/dynamodb'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { workspaceId, channelId, columnId, title, description } = body ?? {}

    if (!workspaceId || !channelId || !columnId || !title || typeof title !== 'string' || !title.trim()) {
      return NextResponse.json(
        { error: 'workspaceId, channelId, columnId and title are required' },
        { status: 400 },
      )
    }
    ...
    const card = await createCard({ workspaceId, channelId, columnId, title: title.trim(), description })

    boardStore.upsertCard(card)
