...
import { assertWorkspaceMember, getBoard } from '../../../lib/features/board/dynamodb'
...
export async function GET(request: NextRequest) {
  try {
    ...
    columns.forEach((col) => boardStore.upsertColumn(col))
    cards.forEach((card) => boardStore.upsertCard(card))

    return NextResponse.json({ columns, cards }, { status: 200 })
  } catch (err: any) {
