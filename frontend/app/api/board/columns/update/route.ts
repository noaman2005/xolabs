...
import { assertWorkspaceMember, updateColumn, getBoard } from '../../../../../lib/features/board/dynamodb'
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { workspaceId, channelId, columnId, updates } = body ?? {}
    ...
    const column = await updateColumn({ workspaceId, channelId, columnId, updates: updates ?? {} })

    if (!column) {
      return NextResponse.json({ error: 'Column not found' }, { status: 404 })
    }

    boardStore.upsertColumn(column)
