...
import { deleteTask, assertWorkspaceMember, getTasks } from '../../../../lib/features/tasks/dynamodb'
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await deleteTask({ workspaceId, channelId, taskId })

    const roomKey = `TASKS#${workspaceId}#${channelId}`

    // Also remove from in-memory room if it exists
    taskStore.deleteTask(workspaceId, channelId, taskId)
